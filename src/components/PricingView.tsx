...
const plans: Array<{ tier: PlanTier; monthlyPrice: number; credits: number; features: string[] }> = [
  { tier: "free", monthlyPrice: 0, credits: 25, features: ["5 starter unlocks", "Creator direct contacts", "Basic creator search"] },
  { tier: "basic", monthlyPrice: 1499, credits: 100, features: ["Creatorly Discovery", "Private creator CRM", "Chrome extension", "Campaign workspace"] },
  { tier: "pro", monthlyPrice: 3499, credits: 250, features: ["Everything in Basic", "Creator + manager contacts", "Agent and assistant contacts", "Higher contact allowance"] },
];
...
export function PricingView({ viewer, navigate }: { viewer?: Viewer | null; navigate(route: AppRoute): void; refresh(): void }) {
  ...
  return <main className="workspace pricing-workspace">
    ...
    <section className="plan-grid">{plans.map((plan) => {
      ...
      return <article className={plan.tier === "pro" ? "is-featured" : ""} key={plan.tier}>
        {plan.tier === "pro" ? <span className="popular-label"><Crown size={13}/> Manager access</span> : null}
        <p className="eyebrow">{plan.tier}</p><h2>{formatRupees(headlinePrice)}<small>{annual && plan.monthlyPrice > 0 ? "/month equivalent" : "/month"}</small></h2>
        {annual && plan.monthlyPrice > 0 ? <p className="plan-billing-detail">billed annually {formatRupees(annualTotal)}</p> : null}
