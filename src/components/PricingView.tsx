...
export function PricingView({ viewer, navigate, refresh }: { viewer?: Viewer | null; navigate(route: AppRoute): void; refresh(): void }) {
  ...
  return <main className="workspace pricing-workspace"><header className="pricing-intro"><div><p className="eyebrow">Plans and credits</p><h1>Pay for the access you need.</h1><p>DemoPay simulates checkout. No real payment is collected.</p></div><div className="billing-toggle"><button className={!annual ? "is-active" : ""} onClick={() => setAnnual(false)}>Monthly</button><button className={annual ? "is-active" : ""} onClick={() => setAnnual(true)}>Annual <small>2 months free</small></button></div></header>
