...
export function OnboardingView({ viewer, navigate, refresh }: { viewer: Viewer; navigate(route: AppRoute): void; refresh(): Promise<void> }) {
  ...
  async function finish() {
    ...
    try {
      await workspaceData.completeWorkspaceOnboarding({
        ...
        role,
        goals: selectedGoals,
        inviteEmail: invite.trim() || undefined,
      });
      await refresh();
  ...
  return <main className="onboarding-page workspace-onboarding"><header><strong>Creatorly</strong><span>Step {step} of 5</span></header><div className="onboarding-progress"><i style={{ width: `${step * 20}%` }}/></div><div className="onboarding-layout"><aside><p className="eyebrow">Workspace setup</p><h2>Build the operating system around your creator team.</h2><ol>{labels.map((label, index) => <li className={index + 1 === step ? "is-current" : index + 1 < step ? "is-done" : ""} key={label}><span>{index + 1 < step ? <Check size={13}/> : index + 1}</span>{label}</li>)}</ol></aside><section className="onboarding-workarea">
    ...
    {step === 3 ? <div><span className="onboarding-glyph"><Users/></span><p className="eyebrow">Team and ownership</p><h1>Set the first campaign owner.</h1><p>You can invite teammates now or continue and do it from settings later.</p><div className="onboarding-form-grid"><label>Your day-to-day role<select value={role} onChange={event => setRole(event.target.value as WorkspaceRole)}><option value="owner">Owner</option><option value="admin">Admin</option><option value="manager">Campaign manager</option><option value="contributor">Contributor</option><option value="reviewer">Reviewer</option></select></label><label>Teammate email <small>Optional</small><input type="email" value={invite} onChange={event => setInvite(event.target.value)} placeholder="teammate@agency.com"/></label></div><p className="onboarding-note">You keep workspace owner access. Invites stay pending until a real email provider is connected.</p></div> : null}
