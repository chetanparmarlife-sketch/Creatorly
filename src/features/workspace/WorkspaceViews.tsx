...
import { ArrowRight, CalendarClock, Check, ChevronRight, CircleDollarSign, FolderKanban, Plus, Search, Sparkles, Users } from "lucide-react";
...
export function CreatorsWorkspace({ workspace, navigate }: { workspace: WorkspaceSummary; navigate(route: AppRoute): void }) {
  const store = useWorkspaceData(); const [items, setItems] = useState<SavedCreator[]>([]); const [filter, setFilter] = useState("");
  const load = useCallback(() => store.listSavedCreators(workspace.id).then(setItems), [store, workspace.id]); useEffect(() => { void load(); }, [load]);
  const visible = useMemo(() => items.filter(item => `${item.creator.displayName} ${item.creator.handle}`.toLowerCase().includes(filter.toLowerCase())), [filter, items]);
  ...
  return <main className="workspace ops-page"><PageHeader eyebrow="Relationship workspace" title="Creator CRM" copy="Every saved creator, owner, stage, and next action your team is responsible for." action={<button className="button button-primary" onClick={() => navigate({ name: "discover" })}><Plus size={16}/> Add creators</button>}/>
    <div className="crm-toolbar"><label><Search size={16}/><span className="sr-only">Search saved creators</span><input value={filter} onChange={event => setFilter(event.target.value)} placeholder="Search saved creators"/></label><span>{visible.length} creators</span></div>
    <section className="ops-table-card"><div className="crm-head"><span>Creator</span><span>Stage</span><span>Owner</span><span>Next action</span><span>Priority</span></div>{visible.length ? visible.map(item => <article className="crm-row" key={item.id}><button className="creator-cell" onClick={() => navigate({ name: "creator", creatorId: item.creator.id })}><span className="creator-monogram">{item.creator.displayName[0]}</span><span><strong>{item.creator.displayName}</strong><small>{item.creator.handle}</small></span></button><select aria-label={`${item.creator.displayName} stage`} value={item.relationshipStage} onChange={event => update(item, event.target.value as CampaignStage)}>{CAMPAIGN_STAGES.map(stage => <option key={stage} value={stage}>{stageLabel(stage)}</option>)}</select><span>{item.ownerName}</span><span>{item.nextAction ?? "Add next action"}</span><span className={`priority priority-${item.priority}`}>{item.priority}</span></article>) : <Empty icon={<Users/>} title="Your creator roster is empty" copy="Save creators from discovery to build a shared relationship history." action="Discover creators" onAction={() => navigate({ name: "discover" })}/>}</section>
  </main>;
}
