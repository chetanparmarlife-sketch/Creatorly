...
function statusLabel(status: CreatorClaim["status"]) {
  return ({ draft: "Draft", enrichment_pending: "Public data queued", ready_for_verification: "Ready to verify", verification_pending: "Verification pending", verified: "Verified", review_required: "In review", published: "Published", rejected: "Changes required", suspended: "Suspended" })[status];
}
...
function ProfilePreview({ claim }: { claim: CreatorClaim }) {
  return <article className="claim-preview">
    ...
    <dl>
      ...
      {claim.enrichedFollowerCount !== undefined ? <div><dt>Apify audience</dt><dd>{claim.enrichedFollowerCount.toLocaleString()} followers</dd></div> : null}
...
export function ClaimProfileView({ authenticated, navigate }: { authenticated: boolean; navigate(route: AppRoute): void }) {
  ...
  if (!authenticated || !claim) return <main className="claim-public">
    <header className="claim-public-header"><button className="brand-button" onClick={() => navigate({ name: "landing" })}><Logo/></button><button className="button button-secondary" onClick={() => navigate({ name: authenticated ? "claimProfile" : "login" })}>{authenticated ? "My claim" : "Sign in"}</button></header>
    <section className="claim-hero">
      <p className="eyebrow">Creator-owned profiles</p>
      <h1>Own the profile <span>brands see.</span></h1>
      <p>Add your Instagram handle. Creatorly will collect public profile data, then you add the business details only you know.</p>
      ...
      <p className="claim-no-oauth"><ShieldCheck size={16}/> No Instagram password, account connection, or Meta sign-in.</p>
    </section>
...
function ClaimWizard({ claim, step, setStep, refresh, navigate, error, setError }: { claim: CreatorClaim; step: number; setStep(value: number): void; refresh(): Promise<void>; navigate(route: AppRoute): void; error: string; setError(value: string): void }) {
  ...
  const completedThrough = useMemo(() => claim.status === "published" ? 5 : claim.status === "review_required" ? 5 : claim.status === "verification_pending" ? 4 : claim.status === "ready_for_verification" ? 3 : claim.categories.length && claim.languages.length ? 2 : 0, [claim]);
  ...
  return <main className="claim-page">
    <header className="claim-topbar"><button className="brand-button" onClick={() => navigate({ name: "landing" })}><Logo/></button><div><span className="claim-status"><i/>{statusLabel(claim.status)}</span><button className="button button-secondary" onClick={() => navigate({ name: "discover" })}>Brand workspace</button></div></header>
    <div className="claim-layout">
      ...
      <section className="claim-workspace">
        ...
        {step === 4 ? <StepShell icon={<ShieldCheck/>} kicker="Step 5 of 6" title="Prove profile ownership" description="Choose one proof. Creatorly reviews the result before publishing your profile."><div className="verification-options"><VerificationOption title="Instagram bio code" description="Temporarily add a Creatorly code to the public Instagram bio." selected={claim.verificationMethod === "instagram_bio"} disabled={busy} onClick={() => void issue("instagram_bio")}/><VerificationOption title="Business email" description="Use the business email listed on the public profile." selected={claim.verificationMethod === "business_email"} disabled={busy || !form.businessEmail} onClick={() => void issue("business_email")}/><VerificationOption title="Website backlink" description="Add a temporary Creatorly link to your public website." selected={claim.verificationMethod === "website_backlink"} disabled={busy || !form.websiteUrl} onClick={() => void issue("website_backlink")}/></div>{claim.verificationCode ? <div className="claim-code"><p>Your temporary verification code</p><strong>{claim.verificationCode}</strong><small>{claim.verificationMethod === "instagram_bio" ? "Place this in your Instagram bio, then submit it for review." : claim.verificationMethod === "business_email" ? `Creatorly will confirm this proof using ${form.businessEmail}.` : "Place a Creatorly link or this code on your website, then submit it."}</small></div> : null}<Footer busy={busy} onBack={() => setStep(3)} nextLabel={claim.status === "verification_pending" || claim.status === "review_required" || claim.status === "published" ? "Continue" : "I added the proof"} disabled={!claim.verificationCode} onNext={() => claim.status === "verification_pending" || claim.status === "review_required" || claim.status === "published" ? setStep(5) : void submitVerification()}/></StepShell> : null}
        ...
        {error ? <p className="form-error claim-error" role="alert">{error}</p> : null}
      </section>
    </div>
  </main>;
}
