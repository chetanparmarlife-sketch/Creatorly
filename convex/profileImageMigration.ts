...
export const loadFacebookPage = internalQuery({
  args: { cursor: v.union(v.string(), v.null()) },
  handler: async (ctx, args): Promise<YouTubeMigrationPage> => {
    const result = await ctx.db
      .query("creators")
...
export const commitFacebookBatch = internalMutation({
  args: {
    successful: v.array(v.object({
      creatorId: v.id("creators"),
      storageId: v.id("_storage"),
...
export const copyFacebookImages = internalAction({
  args: {},
  handler: async (ctx): Promise<{ scanned: number; migrated: number; failed: number }> => {
    let cursor: string | null = null;
    let scanned = 0;
    ...
    let failed = 0;
    for (;;) {
      const page: YouTubeMigrationPage = await ctx.runQuery(internal.profileImageMigration.loadFacebookPage, { cursor });
      scanned += page.scanned;
      const successful: CopySuccess[] = [];
      for (let index = 0; index < page.creators.length; index += 50) {
        const group = page.creators.slice(index, index + 50);
      ...
      if (successful.length) {
        const result = await ctx.runMutation(internal.profileImageMigration.commitFacebookBatch, { successful });
        migrated += result.committed;
      }
      if (page.isDone) break;
      cursor = page.continueCursor;
