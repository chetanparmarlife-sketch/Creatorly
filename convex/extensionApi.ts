...
export const profile = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!creator) return { authenticated: true as const, found: false as const, creditBalance: account.user.creditBalance ?? 0, currentPlanTier: account.user.currentPlanTier ?? "free", workspace: workspacePayload(workspace), isSaved: Boolean(saved), savedCreatorId: saved?._id };
    ...
    return {
      ...
      creditBalance: account.user.creditBalance ?? 0, currentPlanTier: account.user.currentPlanTier ?? "free", availableContactCount: permitted.length, hiddenProContactCount, pendingContactCount, isUnlocked: Boolean(active && hasOpenableContact), expiresAt: active && hasOpenableContact ? active.expiresAt : undefined,
