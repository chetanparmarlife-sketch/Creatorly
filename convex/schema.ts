...
export default defineSchema({
  ...
  users: defineTable({
    ...
    persona: v.optional(v.union(v.literal("buyer"), v.literal("creator"), v.literal("both"))),
  })
    .index("email", ["email"])
    .index("phone", ["phone"]),
  creators: defineTable({
    ...
    metricProvenance: v.optional(v.union(v.literal("supplied"), v.literal("live"), v.literal("apify"))),
  ...
  creatorClaims: defineTable({
    ...
    normalizedInstagramHandle: v.string(),
    ...
    contactPreference: v.union(v.literal("direct"), v.literal("manager_only"), v.literal("not_contactable")),
    ...
    enrichmentStatus: v.optional(v.union(v.literal("queued"), v.literal("running"), v.literal("complete"), v.literal("failed"))),
    enrichmentProvider: v.optional(v.literal("apify")),
    enrichmentActorId: v.optional(v.string()),
    enrichmentRunAt: v.optional(v.number()),
    enrichedFollowerCount: v.optional(v.number()),
    enrichedFollowingCount: v.optional(v.number()),
    enrichedPostCount: v.optional(v.number()),
    enrichedEngagementRatePercent: v.optional(v.number()),
    enrichedProfileImageUrl: v.optional(v.string()),
    enrichedBusinessCategoryName: v.optional(v.string()),
    enrichedIsVerified: v.optional(v.boolean()),
    enrichedIsPrivate: v.optional(v.boolean()),
    enrichedIsBusinessAccount: v.optional(v.boolean()),
    verificationMethod: v.optional(v.union(v.literal("instagram_bio"), v.literal("business_email"), v.literal("website_backlink"))),
    ...
    .index("by_handle", ["normalizedInstagramHandle"])
    .index("by_creator", ["creatorId"])
    .index("by_status", ["status"]),
  creatorClaimAssets: defineTable({
    ...
    .index("by_user", ["userId"]),
  creatorClaimAuditEvents: defineTable({
    claimId: v.id("creatorClaims"),
